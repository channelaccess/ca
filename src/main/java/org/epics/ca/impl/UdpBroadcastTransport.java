 ...
 */
public class UdpBroadcastTransport implements ReactorHandler, Transport
{
...
/*- Private attributes -------------------------------------------------------*/
   ...
   // Get Logger
   private static final Logger logger = LibraryLogManager.getLogger( UdpBroadcastTransport.class );
...
/*- Constructor --------------------------------------------------------------*/
    ...
    */
   public UdpBroadcastTransport( ContextImpl context,
   ...
   private void processRead()
   {
      try
      {
         while ( true )
         {
            ...
            final InetSocketAddress fromAddress = (InetSocketAddress) channel.receive (receiveBuffer);
            ...
            while ( receiveBuffer.limit () - receiveBuffer.position () >= Constants.CA_MESSAGE_HEADER_SIZE )
            {
               ...
               final int endOfMessage = pos + header.payloadSize;
               ...
               {
                  logger.warning( String.format( "'%s': unexpected exception caught while processing CA message over UDP from '%s'", th, fromAddress ) );
               }
               finally
               {
