...
public final class Messages
{
                                            ...
                                            int parameter1, int parameter2 )
   {
      ...
      final boolean useExtendedHeader = payloadSize >= 0xFFFF || dataCount >= 0xFFFF;

      // check if supported by current transport protocol revision
      if ( useExtendedHeader && transport.getMinorRevision () < 9 )
      {
         throw new IllegalArgumentException( "Out of bounds." );
      }

      final int requiredSize = useExtendedHeader ?
            Constants.CA_EXTENDED_MESSAGE_HEADER_SIZE :
            Constants.CA_MESSAGE_HEADER_SIZE;

      final ByteBuffer buffer = transport.acquireSendBuffer (requiredSize + payloadSize);
   ...
   public static boolean generateRepeaterRegistration( ByteBuffer buffer )
   {
    ...
    */
   public static boolean generateSearchRequestMessage( Transport transport, ByteBuffer buffer, String name, int cid )
   {
      Validate.notNull( transport );
      Validate.notNull( buffer );
    ...
    */
   public static void generateVersionRequestMessage( Transport transport, ByteBuffer buffer, short priority,
                                                     int sequenceNumber, boolean isSequenceNumberValid )
   {
      Validate.notNull( transport );
      Validate.notNull( buffer );

      final short isSequenceNumberValidCode = isSequenceNumberValid ? (short) 1 : (short) 0;
    ...
    * @param buffer the buffer.
    */
   public static void generateEchoMessage( Transport transport, ByteBuffer buffer )
   {
      Validate.notNull( transport );
      Validate.notNull( buffer );
   ...
   public static void hostNameMessage( Transport transport, String hostName )
   {
      ...
      try
      {
         final ByteBuffer buffer = startCAMessage (transport,
                                             (short) 21,
   ...
   public static void userNameMessage( Transport transport, String userName )
   {
      ...
      final int unalignedMessageSize = Constants.CA_MESSAGE_HEADER_SIZE + userName.length () + 1;
      final int alignedMessageSize = calculateAlignedSize (8, unalignedMessageSize);
   ...
   public static void createChannelMessage( Transport transport, String channelName, int cid )
   {
      ...
      final int unalignedMessageSize = Constants.CA_MESSAGE_HEADER_SIZE + binaryNameLength;
      final int alignedMessageSize = calculateAlignedSize (8, unalignedMessageSize);
   ...
   public static <T> void writeMessage( Transport transport, int sid, int cid, TypeSupport<T> typeSupport, T value, int count )
   {
      ...
      final int calculatedPayloadSize = typeSupport.serializeSize (value, count);
      final int alignedPayloadSize = calculateAlignedSize (8, calculatedPayloadSize);
